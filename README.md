...
> 主要增加了 **fnOS 应用（`.fpk`）离线封装**，并将界面重做为左侧导航 + 任务列表的布局。
...
| 界面重构 | 界面改为左侧格式导航树 + 顶部工具栏 + 任务列表 + 底部状态栏的布局 |
...
完整的版本更新记录见 **[CHANGELOG.md]**（本次解锁流程重做与相关缺陷修复记录在 `2026-09-23` 条目下）。

[CHANGELOG.md]: ./CHANGELOG.md

更早之前的改动见上文 **四、代码层面修复记录**（Element UI 按需注册、joox-crypto 本地依赖、fnOS 服务被误杀、多余提示与更新弹窗）。
